  ...
  return (
    <div className="w-full h-full bg-gray-50 overflow-hidden p-4">
      <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] md:gap-6 h-full">
        ...
        <div className="w-full flex flex-col overflow-hidden">
          {mostRecentArticle ? (
            <>
              <div className="p-3 md:p-4 bg-white">
                <Link href={`/article/${mostRecentArticle.slug}`}>
                  <article className="cursor-pointer">
                    ...
                    {/* Clamp the main excerpt to two lines to avoid overflow */}
              ...
              {/* images removed from middle column per design */}
            </>
          ) : (
        ...
        {/* Secondary column - lists and CTA, stacks beneath main on mobile */}
        <div className="w-full flex flex-col h-full overflow-hidden">
                ...
                otherArticles.map((article) => (
                  <Link key={article.id} href={`/article/${article.slug}`}>
                    <article className="border-b border-gray-200 pb-3 last:border-b-0 cursor-pointer hover:bg-gray-50 p-2 rounded transition-colors">
                      ...
                      {/* Shorter secondary excerpt and clamp to two lines */}
            ...
            <div className="mt-3 bg-red-50 p-2 rounded border border-red-200">
              <p className="text-xs text-gray-700 mb-1">Share your NYC perspective</p>
              <button className="bg-red-600 text-white px-3 py-1 rounded text-xs hover:bg-red-700 transition-colors font-semibold">
                Submit Entry
              </button>
            </div>
          </div>
